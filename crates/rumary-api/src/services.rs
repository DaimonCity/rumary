...
use rumary_dto::domain::api::{LoginOutcome, User};
...
pub trait AuthProvider: Send + Sync {
    ...
    async fn authenticate_access_token(
        &self,
        token: &str,
    ) -> Result<AuthenticatedUser, Self::Error>;
    ...
    async fn issue_tokens(&self, user_uuid: Uuid) -> Result<SessionTokensResponse, Self::Error>;

    fn encode_access_token(&self, user: &User) -> Result<String, Self::Error>;
}
